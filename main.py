...
obfuscate = dict(map(lambda map,dict:(map,dict),['(https://pyobfuscate.com)*(RSA)'],['''P5%sZ);Jr<fz&nb!5<EaKky^%FxgoYvI}+JmoB+?zcKvzX2>Gae&#j`U@Q>h)?x-OTvdhoJufOkovDrM3qbEsHQgk7?eZIp{1ZiLJ1av<Wt<2&DyGT}`~*fAYB&K)!KwVri;BBH-X^}W9O?)oCA1nSzg^)WUH*~oUddYd>29JoXw2;&Q71mFlZg?=kr{!ReC&7)!KUK`W6|g^9$(vP*I<`qxvBCx>t!zri&PbzCp}$x<R2wW{-6&NEHlgryWYo8HpFOb'''.replace('\n','')]))
...
def gettodolist(bearer):
    ...
    try:
        if hasil["status"] == 200:
            now = datetime.now()
            timenow = now.strftime("%H:%M:%S")
            print(Colorate.Color(Colors.yellow,"[{}] ".format(timenow)) + Colorate.Color(Colors.green,"[+] Success."))

            groups = hasil['data']['groups']

            formatted_details = []
            groups = hasil['data']['groups']

            for group in groups:
                todolists = group['todolists']
                for todolist in todolists:
                    todolist_details = todolist['todolist_details']
                    for detail in todolist_details:
                        if detail['type'] == "YES-NO":
                            formatted_detail = {
                                "todolist_detail_id": str(detail['id']),
                                "value": "yes", 
                                "d_date": "payload",  
                                "poin": detail['poin']
                            }
                            ...
                            formatted_detail = {
                                "todolist_detail_id": str(detail['id']),
                                "value": str(detail['max_value']),  
                                "d_date": "payload",  
                                "poin": detail['poin']
                            }
                        formatted_details.append(formatted_detail)

            todolistf = {"todolist_values": formatted_details}
            token = bearer
    except KeyboardInterrupt:
        now = datetime.now()
        timenow = now.strftime("%H:%M:%S")
    ...
    try:
        ...
        if hasil1.status_code == 422:
            ...
            if status == 200:
                ...
                print(Colorate.Color(Colors.yellow,"[{}] ".format(timenow)) + Colorate.Color(Colors.green,"[+} Success => ") ,Colorate.Color(Colors.white, "{}".format(bearer)))
                delay(1)
                gettodolist(bearer)
            elif status == 401:
                now = datetime.now()
